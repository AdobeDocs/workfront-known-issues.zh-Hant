...
+++**2022 年 11 月 17 日維護更新**

**放在 [!UICONTROL 回收筒] 如果在移動任務或問題時未選中**

*文件*

現在，當您取消選取 [!UICONTROL 檔案] 在移動任務或問題的過程中的選項、附加到任務的文檔或問題將放在 [!UICONTROL 回收筒] 30天。 如有需要，管理員可以還原。 在移動過程中取消選擇「文檔」的用戶將在 [!UICONTROL 移動任務] 或 [!UICONTROL 移動問題] 框。 在此增強之前，文檔會被永久刪除。
...
**「[!UICONTROL 新校樣收件者]」文字方塊太小**

*校樣*

使用者正在檢視一個校樣並嘗試從[!UICONTROL 分享]索引標籤分享校樣時，「[!UICONTROL 新校樣收件者]」文字方塊太小。使用者可以輸入名稱，但由於方塊太小，文字換行方式造成可讀性低。
...
+++**[!DNL Workfront Fusion]2022 年 11 月 3 日維護更新**
...
當使用者作為檢閱者或請求者新增到校樣時，他們將獲得「[!UICONTROL 訪客]」權限設定檔，該設定檔不應使用 [!DNL Workfront Proof] 授權。但是，當新增使用者時，使用的 [!DNL Workfront Proof] 授權的數量會增加。

+++

+++**2022 年 10 月 11 日維護更新**
...
當正在 [!DNL Workfront] 內檢視校樣的使用者關閉校樣，他們未返回到開啟校樣之前所在的頁面。相反地，他們被重新導向到 [!DNL Workfront] 的另一個頁面。

**無法在[!DNL Workfront]** 中開啟校樣

*校樣*

當使用者在 [!DNL Workfront] 中檢視文件並嘗試開啟校樣，校樣未開啟且使用者返回[!UICONTROL 文件詳細資訊]頁面。
...
當使用者檢視具有多個頁面的校樣時，使用者可以看到頁面的縮圖，但頁面無法在主檢視器中開啟。
...
+++**[!DNL Workfront Scenario Planner]2022 年 9 月 15 日維護更新**
...
**移除了透過電子郵件將校樣發送到 dropzone 的功能**

*[!DNL Workfront Proof]*

自 2022 年 9 月 8 日星期四起，我們已移除將校樣透過電子郵件發送到獨立版中的 dropzone 的功能[!DNL Workfront Proof]產品。

您仍然可以透過其他方式使用 dropzones 向您的帳戶提交新的校樣和新版本的校樣，而無需登入您的帳戶。如需詳細資訊，請參閱[拖放區](https://experienceleague.adobe.com/docs/workfront/using/workfront-proof/work-with-proofs-in-wf-proof/create-proofs-and-files/dropzone.html?lang=zh-Hant)。
...
建立新校樣時，如果使用者上傳大於 1.5GB 的檔案，則檔案名稱將變為紅色，且無法建立校樣。
...
當使用者正在查看互動式校樣並按一下連結或包含連結的按鈕時，使用者未導向到連結或按鈕連結到的頁面。
...
當使用者在校樣評論中被標記時，他們未收到有關評論的電子郵件通知。
...
**建立、移動或封存校樣時，[!DNL Workfront Proof] 區域未更新**

*[!DNL Workfront]校樣*

校樣正在經歷索引延遲。這可能會經由以下方式影響使用者體驗：
...
本月早些時候，作為修補檔的一區段[!DNL Workfront]生產環境，我們修復了校樣郵件通知系統的一些錯誤。此變更在發佈時未在維護更新中傳達。我們已將以下資訊新增到 [2022 年 6 月 2 日維護更新](#maintenance-update-on-june-2-2022)：

由於這些錯誤修復，用於發送校樣通知的電子郵件地址已變更。

以前，校樣電子郵件地址包含您組織的子域。例如，通知@[公司網域名稱].my.workfront.com

現在，校對電子郵件地址不再包含組織子域。所有校樣電子郵件通知將來自以下地址：notification@my.workfront.com
...
* [!UICONTROL 「透過標記共用校樣]」功能
* 校樣作用 ([!UICONTROL 檢閱者]、[!UICONTROL 核准者])
...
當使用者在查看校樣的[!UICONTROL 校樣工作流程]並嘗試將自己拖到校樣的不同階段時，使用者的名稱會快速恢復到原始階段，而未新增到所需的階段。
...
+++**[!DNL Workfront Scenario Planner]2022 年 6 月 9 日維護更新**
...
+++**[!DNL Workfront Fusion]2022 年 6 月 9 日維護更新**
...
當使用者嘗試編輯 [!DNL Workfront Proof] 中使用者角色的「[!UICONTROL 解決評論並套用動作]」或「[!UICONTROL 透過標記共用校樣]」權限，變更未儲存。使用者收到範本已更新的通知，但如果使用者再次打開角色權限，他們可以看到變更未儲存。
...
**無法從[!DNL PowerPoint]** 建立校樣

*[!DNL Workfront Proof]*

當使用者嘗試從 [!DNL PowerPoint] (包括圖表) 建立校樣時，校樣建立失敗。

**無法從 [!UICONTROL Word] 檔案建立校樣**

*[!DNL Workfront Proof]*

當使用者嘗試從 [!DNL Word] 文件 (包括圖表) 建立校樣時，校樣建立失敗。

**共用校樣時無法新增自訂訊息**

*[!DNL Workfront Proof]*

當使用者查看校樣時，打開[!UICONTROL 共用校樣]區域，並選擇[!UICONTROL 新增自訂訊息]按鈕，使用者無法在打開的文字方塊中輸入內容。當使用者嘗試在此方塊中鍵入時，該方塊立即消失。

**無法關閉校樣**

*[!DNL Workfront Proof]*

當使用者正在查看一個校樣並嘗試關閉它時，校樣右上角的 X 會遺失。
...
當使用者嘗試建立校樣時，選項或按鈕的文字不可見。因此，使用者不知道每個選項或按鈕代表什麼，也無法設定校樣。
...
現在，只需按一下任務欄或問題欄[!UICONTROL 工作負載平衡器]打開摘要面板。在此更新之前，您必須按一下[!UICONTROL 打開摘要]工具列中的圖示，然後按一下任務或問題。這已被校樣是一種令人困惑的經歷，現在已得到糾正。或者，您可以按一下任務或問題名稱旁邊的[!UICONTROL 更多]選單，然後按一下[!UICONTROL 打開摘要]。
...
當使用者對校樣進行評論並嘗試打開[!UICONTROL 文字]工具時，該工具未打開且使用者看到以下訊息：
...
我們正在調整校樣電子郵件通知的發送方式。現在，通知將發送到使用者的主要電子郵件地址，而不是系統產生的別名電子郵件。
...
**新增到校樣的使用者無法存取該校樣的工作項[!DNL Workfront]**

*校樣*

如果將使用者新增到校樣工作流中的某個階段，則該使用者未新增到文件共用中，並且未獲得對中校樣工作項的權限[!DNL Workfront]。當使用者在[!DNL Workfront]並嘗試打開附有校樣的工作項，他們會看到以下訊息：

「[!UICONTROL 您沒有足夠的存取權可檢視此 (物件)]」

此問題特定於已建立的校樣和事後新增的使用者。在建立校樣之前將使用者新增到工作流程按預期運作。
...
當使用者將另一個使用者新增到校樣時，該使用者在校樣上的角色設定為「[!UICONTROL 唯讀]」，無論使用者的實際校樣角色為何。
...
解決方法：
將使用者在其設定檔中的校樣角色設定為其他內容，然後重設為正確的角色。
...
**[!DNL XLS]和 [!DNL XLSX] 檔案大小限制暫時減少到 100 MB 以用於校樣**

*校樣*

為了解決安全問題，在建立校樣時暫時將 [!DNL XLS] 和 [!DNL XLSX] 檔案的最大檔案大小限制為 100 MB。
...
+++**[!DNL Workfront Fusion]2022 年 2 月 18 日維護更新**
...
**文件和校樣的「[!UICONTROL 更多]」選單問題**

*文件*

當使用者在 [!DNL Workfront Classic] 文件清單中選擇文件或校樣，然後按一下「[!UICONTROL 更多]」時，他們可能會看到以下問題之一：
按鈕沒有反應
...
**已停用和重新啟動的使用者未出現在[!UICONTROL 校樣聯絡人]**&#x200B;中
...
+++**[!DNL Workfront Fusion]2022 年 1 月 14 日維護更新**
...
當使用者在[!UICONTROL 文件]區域中，對校樣本身所做的任何評論都未顯示在[!UICONTROL 更新]文件的區域。
