...
>[!NOTE]
>
>此問題已在 2022 年 10 月 13 日的預覽版中修正。它已於2022年10月27日正式投產。

當使用者在圖釘上選擇「[!UICONTROL 移除圖釘]」選項時，會收到有關移除作業訊息，並嘗試透過按一下訊息中的「[!UICONTROL 還原]」來放回圖釘，圖釘不會在頂部導覽中被放回，也不會新增到[!UICONTROL 更多圖釘]清單 ([!UICONTROL 圖釘]區域中的三點選單) 下的圖釘清單中。

如果使用者嘗試透過移至頁面並釘選它來重新釘選頁面，圖釘不會建立，並且使用者無法釘選頁面。

**在[!DNL Safari]瀏覽器**&#x200B;中使用分享連結時在[!UICONTROL 工作負載均衡器]中列出所有使用者
...
當使用者大量編輯任務或問題，並在指派個人後指派團隊，指派不會儲存且使用者會看到下列錯誤：
