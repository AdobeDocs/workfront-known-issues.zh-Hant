...
當不是Workfront管理員的使用者檢視自訂表單時，其中欄位從表單僅限管理員區段中的資料提取，該欄位不顯示計算值。 如果使用者執行重新計算運算式，則會顯示值，但在重新整理時遺失。

Workfront管理員可如預期看到欄位值。
...
當校訂的評論中提及使用者時，該提及未包含在首頁的提及小元件中。

#### 登入

**登入後未載入Workfront**

使用者登入Workfront時，Workfront未載入。 如果使用者重新整理頁面，Workfront會如預期載入。

#### 報告

Workfront計畫欄位未出現在畫布控制面板Report Builder中

當使用者在畫布儀表板中使用產生器並嘗試參考Workfront Planning中的欄位時，這些欄位未顯示在產生器中。
