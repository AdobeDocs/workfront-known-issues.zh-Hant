...
**無法新增「estimateByHours」或「hoursPerPoint」工作列位**

使用者嘗試將欄位「estimateByHours」或「hoursPerPoint」新增至表單產生器中的自訂表單時，會顯示錯誤訊息：「此自訂運算式無效，請再試一次。」

**顯示邏輯在自訂表單中無法運作**

當使用者在具有顯示邏輯的自訂表單中輸入資訊時，根據顯示邏輯應出現的欄位未出現。

#### 首頁

**貢獻者顯示「處理它」按鈕無法運作**

投稿人使用者可以存取首頁我的工作Widget中的非功能「處理」按鈕。

#### 報告

**自訂季度未出現在報告提示中**

當使用者執行具有日期型提示的報告時，自訂季度未出現在提示的選擇清單中。
...
在包含「類別名稱」欄位的報表中檢視清單時，無法按一下「類別名稱」欄的內容以開啟對應的自訂表單。

**報告中的群組自訂顏色無法運作**

在Report Builder中指定群組的自訂顏色時，執行報表時不會顯示顏色選取範圍。

報告&#x200B;**長載入時間**

報告需要長時間才能載入。這樣會影響檢視報告，以及在報告產生器中編輯和建立報告。

報告提示中計算的「日期/時間」欄位缺少&#x200B;**時間欄位**

當執行報表時，自訂日期/時間欄位用作提示，時間選擇欄位在提示介面中遺失。

**無法檢視共用報告/儀表板**

當使用者嘗試存取已與他們共用的報告或儀表板時，無法存取該報告或儀表板。
