...
+++**[!DNL Adobe Workfront Fusion]2023 年 12 月 21 日維護更新**
...
當使用者設定模組且此模組需要所連接帳戶的資料 (例如選取記錄) 時，此模組無法擷取資料，且資料要求逾時。

+++

+++**2023 年 12 月 14 日維護更新**
...
**核准卡在「[!UICONTROL 正等待我的核准]」小工具**

_首頁_

當上傳文件的新版本且舊版本的核准未完成時，文件的舊版本會卡在核准者的「[!UICONTROL 正等待我的核准]」小工具。 由於有新版本，因此核准無法完成，並且無法將其從小工具中移除。

**在任務或問題的展示板檢視中新增工作項目時出現問題**
...
**任務未出現在「[!UICONTROL 我的工作]」小工具**

_[!UICONTROL 首頁]_

當使用者在「[!UICONTROL 首頁]」查看其「[!UICONTROL 我的工作]」小工具時，某些指派給他們的任務未出現在小工具中。 例如，使用者可能會前往專案並看到他們獲指派專案任務，但這些任務並未出現在使用者的「[!UICONTROL 我的工作]」小工具中。
...
**使用情況報告中的預算工時與透過 API 報告的預算工時不相符**
...
**透過 API 或[!DNL Workfront Fusion]** 發表的評論中出現額外的行
...
如果「我的工作」小工具在新首頁頁面，會帶入所有預期的物件。 但是，如果此小工具位於頁面上任何其他小工具下方，則它只會帶入 10 個物件。
...
使用者在[!UICONTROL 待辦事項]小工具上 (屬於新的[!UICONTROL 首頁]體驗) 建立任務時，該任務出現在使用者的時程表上。 即使任務沒有記錄時間且並未釘選個人專案，仍會發生這種情況。

+++

+++**於 2023 年 11 月 3 日維護更新 (Hot Fix)**

**當子任務移至父任務下方時，子任務未依順序顯示**
...
共用要求時，如果修改了共用要求的使用者存取層級，則清單中該使用者上方的使用者將成為該使用者。

例如，如果與使用者 A 和使用者 B 共用要求，並修改使用者 B 的存取權限，則使用者 A 會變更為使用者 B，而且清單中現在有兩個使用者 B。 另外，僅修改了最上層使用者 B 的存取權限。

**任務標題發生「[!UICONTROL 糟糕]」錯誤**

_任務_

當使用者檢視任務時，任務標題未包含任何資訊。 使用者反而看到以下錯誤訊息：
...
**篩選器未套用至全新[!UICONTROL 首頁]體驗中的小工具**

_[!UICONTROL 首頁]_

當使用者在全新「[!UICONTROL 首頁]」體驗中將篩選器套用至小工具時，小工具顯示了篩選器應排除的項目。

這已在自訂沙箱環境中報告。 預覽和生產環境中相同的小工具如預期進行篩選。
...
當使用者在校訂檢視器中加上評論並在校訂上做標記，然後按一下離開後，使用者即再也無法調整標記。
...
當使用者在新的[!UICONTROL 首頁]體驗嘗試從[!UICONTROL 待辦事項]小工具中刪除項目時，該項目不會被刪除，並且使用者會看到錯誤。
...
此更新僅包含次要或較不顯著的錯誤修正。[!DNL Workfront] 當您提交的問題得到修復時，支援人員會通知您。
...
在發生應該觸發時間表計算的事件時，時間表不會重新計算。 這會影響在變更時發生的重新計算，以及已排程的重新計算。 這可能會影響工作量平衡工具的準確性。
...
_報告_
當使用者執行任務報告時，報告在載入時似乎正確排序，但當它完成載入時，使用者看到報告未正確排序。
...
您現在可以在編輯評論時編輯附加到評論的影像。 Workfront Goals 的更新區段有提供此更新，當啟用評論測試版體驗時，問題的更新區段也會提供此更新。
...
當使用者在校訂檢視器中檢視校訂並嘗試使用聚合線工具加上評論時，該工具不會標記校訂。
...
>[!NOTE]
>
>此功能於 2023 年 4 月 19 日發佈到預覽版，並於 2023 年 4 月 20 日發佈到生產環境。

_更新_

現在，如果您在撰寫附影像訊息的過程中離開目標的更新頁面，當您返回時該訊息和影像將被保留。 在本次更新前，未提交的評論將會保留，但影像會刪除。 目標的更新區段有提供此更新，當新的評論 Beta 體驗已啟用時，問題的更新區段也會提供此更新。
...
現在，當有人新增評論或回覆，或是從更新區域刪除評論時，您將能即時看到新評論，或是評論已刪除的指示，不會有任何延遲。 目標的更新區段有提供此更新，當新的評論 Beta 體驗已啟用時，問題的更新區段也會提供此更新。
...
**在問題 (新的評論 Beta 體驗) 和[!UICONTROL 目標]**&#x200B;的[!UICONTROL 更新]區段顯示螢幕可見區域外的新評論

_更新_

我們在[!UICONTROL 更新]區段新增了通知橫幅，以便在螢幕可見區域外出現新的項目評論時通知使用者。 目前在目標的[!UICONTROL 更新]區段有提供此更新，當新的評論 Beta 體驗已針對問題啟用時，問題的更新區段也會提供此更新。
...
為了修正[!UICONTROL 桌面校訂檢視器]的評論問題，我們部署了新版本的桌面校訂檢視器。
...
* 開啟評論 (Shift+F2 / Shift+F2)
* 儲存評論 (Cmd+Enter / Ctrl+Enter)
...
**從[!DNL Outlook]** 提交要求時發生錯誤

_整合_

當使用者嘗試透過 [!DNL Outlook] 電子郵件提交具有附件的要求時，一個或多個附件沒有上傳，且使用者會看到以下錯誤：

「[!UICONTROL 發生以下錯誤：具有控點 xxxx 的檔案不存在。]」

僅當透過要求佇列或在建立要求時，手動指派新要求才會發生這種情況。
...
當擁有工作、審閱或申請許可的使用者嘗試編輯使用者設定時，打開的快顯視窗為空白，並無法進行任何編輯。 如果要退出快顯視窗，使用者必須重新整理頁面。
...
**[!UICONTROL 工作量平衡工具] 顯示與專案/任務/問題無關的時數**

_[!UICONTROL 工作負載平衡器]_

當使用者查看 [!UICONTROL 工作量平衡工具]時，他們會看到為使用者記錄的時數，這些時數與任何專案、任務或問題無關，也不會記錄為[!UICONTROL 一般]時數。 這些時數可能僅顯示在 4 週或 6 週的檢視中。
