...
當使用者登入[!DNL Workfront Proof]查看資料夾，資料夾顯示為空。如果使用者稍後再檢查，則校訂是可見的。
...
當使用者打開列印摘要以列印校訂時，會出現標題，但摘要本身是空白的。
...
當使用者建立新校訂並嘗試變更設定時，該設定將恢復為以前的設定。
...
當使用者嘗試在經典中上傳新校訂或新版本的校訂時[!DNL Workfront]體驗，新樣張或新版頁面為空白，使用者無法上傳樣張或版本。

+++

+++**2021 年 7 月 29 日維護更新**

**[!UICONTROL 校訂活動]和[!UICONTROL 校訂檢視器設定]頁面一律可用**

_[!DNL Workfront Proof]_

[!UICONTROL 校訂活動]和[!UICONTROL 校訂檢視器]設定頁面現在一律可見，即使使用者無權更新這些頁面。

先前，當具有自訂校訂權限設定檔的使用者瀏覽到文件時，左側的[!UICONTROL 校訂活動]和[!UICONTROL 校訂檢視器設定]頁面並不一定顯示。
...
當使用者在[!DNL Adobe Workfront Classic]上傳檔案名稱中包含句點的新版本校訂，檔案名稱在末尾被截斷。
...
**[!UICONTROL 去校訂]連結不指向評論**

_電子郵件通知_

當使用者收到校訂評論的電子郵件通知並按一下[!UICONTROL 去校訂]，它們被導向到校訂中的錯誤評論，或者它們根本沒有被導向到任何評論。
...
當使用者在新[!DNL Adobe Workfront]體驗是建立一個新的校訂，他們在校訂的一些自訂欄位中輸入一個值，一些以前的欄位的值恢復為預設值，而不是使用者輸入的值。
...
**上的一些短語[!UICONTROL 新校訂]頁面翻譯不正確**

_[!DNL Workfront Proof]_

當使用者導覽到[!UICONTROL 新校訂建立]頁入[!DNL Workfront Proof]並且內容正在被翻譯成英語以外的其他語言，一些短語仍以英語顯示。

**新增到使用者的停用和刪除標籤[!DNL Workfront Proof]**

_[!DNL Workfront Proof]_

[!UICONTROL 停用]和[!UICONTROL 已刪除]使用者標籤已新增到以下區域[!DNL Workfront]校訂：
...
當使用者上傳另一個版本的校訂時[!DNL Workfront]，這[!UICONTROL 公開校訂]和[!UICONTROL 校訂工作流程]缺少連結，使其看起來像是文件而不是校訂。當這些連結遺失時，標籤[!UICONTROL 擱置]也顯示和其他使用者無法產生校訂，而它是在這個[!UICONTROL 擱置]狀態。
...
當使用者在 [!DNL Workfront Proof] 中建立新校訂，並嘗試將電子郵件中的文字貼上到[!UICONTROL 電子郵件通知]區段的[!UICONTROL 訊息]欄位中時，他們無法貼上文字。
...
當使用者建立新版本的校訂並更新校訂名稱時，文件物件[!DNL Workfront]保留原始名稱而不是符合新的校訂名稱。
...
當使用者在[!UICONTROL 首頁]或者[!UICONTROL 我的工作]，已歸檔的校訂出現在清單中。存檔的校訂也出現在報告和儀表板上。
...
[!DNL Workfront]現在為合併的校訂發送一封電子郵件，而不是為包含的每個檔案發送一封電子郵件。
+++
...
**無法在中建立校訂[!DNL Adobe Creative Cloud]一體化**

_Workfront 整合_

當使用者嘗試直接從[!DNL Adobe Creative Cloud]，不產生校訂。
...
當使用者擁有[!UICONTROL 公開共用]在其預設校訂設定中啟用會建立一個校訂，該校訂是在停用共用的情況下建立的。其他使用者看不到[!UICONTROL 共用]按鈕或共用校訂。

**&quot;[!UICONTROL 無法產生校訂]&quot; 建立校訂時發生錯誤**

_[!DNL Workfront Proof]_

當使用者嘗試建立校訂時，校訂未建立，並且使用者會看到以下錯誤訊息：

&quot;[!UICONTROL 校訂未能產生 - 內部錯誤]&quot;
...
如果獨立[!DNL Workfront Proof]帳戶移至共用元件時，當使用者新增新版本的校訂或文件時，可能會出現以下功能：

* 使用者無法刪除使用者[!UICONTROL 工作室校訂]。
* 預設訊息未出現在新版本上。
...
當使用者在校訂上啟用公開連結共用然後上傳新版本的校訂時，未自動在新版本的校訂上啟用公開連結共用。

**[!UICONTROL 核准],[!UICONTROL 變化],[!UICONTROL 拒絕]校訂中缺少按鈕**

_[!DNL Workfront Proof]_

當使用者在校訂檢視器中查看校訂時，[!UICONTROL 核准],[!UICONTROL 變化]，和[!UICONTROL 拒絕]畫面頂部缺少按鈕。
...
當使用者將自訂訊息附加到校訂，然後上傳該校訂的新版本時，自訂訊息未出現在新校訂上。
...
**無論工作流程設定如何，「[!UICONTROL 將此校訂通知收件者]」選項都停用**

_[!DNL Workfront Proof]_

當使用者建立新的校訂，並且沒有手動打開「[!UICONTROL 將此校訂通知收件者]」選項，則未通知預期的收件者。即使在工作流程設定中啟用了該選項也是如此。
...
當使用者從[!UICONTROL 校訂檢視器]，文件的名稱是文件的先前版本，而不是下載的版本。
...
當使用者在校訂評論中被標記並按一下[!UICONTROL 去校訂]電子郵件通知中的連結，它們被導向到校訂而不是特定評論。如果使用者在[!DNL Workfront Classic]，它們被導向到[!UICONTROL 檔案詳細資訊]頁面而不是校訂中的註釋。

**新增到的使用者[!DNL Workfront]階段接收電子郵件通知**

_[!DNL Workfront Proof]_

當不在工作流程中的使用者打開來自[!DNL Workfront]，系統自動建立一個階段，將此使用者新增到校訂中，並發送一個[!UICONTROL 新校訂]電子郵件通知。
...
當使用者建立新校訂並啟用[!UICONTROL 將此校訂通知收件人]選項時，未發送電子郵件通知收件人。
...
當使用者在新[!DNL Workfront]體驗按一下某些文件連結，他們無法存取該文件並看到錯誤訊息「[!UICONTROL 文件不再存在：您可能輸入了錯誤的網址。仔細檢查並再次嘗試輸入地址。]「同樣的錯誤發生在[!UICONTROL 查看詳細資訊]校訂電子郵件通知中的連結。
...
當使用者打開一個資料夾並新增一個新的校訂時，該校訂將上傳到一般[!UICONTROL 文件]物件的區域而不是資料夾內。
...
**&quot;[!UICONTROL 允許透過 URL 或嵌入代碼共用校訂]&quot; 設定變更**

_[!DNL Workfront Proof]_

當使用者建立校訂並取消核取設定時[!UICONTROL 允許透過 URL 或嵌入代碼共用校訂]，產生校訂後再次檢查設定。如果使用者核取設定，則在產生校訂後取消核取。

**[!DNL Mac]使用者無法貼上到校訂檢視器中的文字欄位中**

_[!DNL Workfront Proof]_

當使用者嘗試將文字貼上到校訂檢視器中的某些欄位時，該文字未出現在該欄位中。
...
當使用者透過電子郵件收到下載連結時 ([!UICONTROL 校訂]>[!UICONTROL 列印評論]>[!UICONTROL  PDF 格式]) 並且他們匯出檔案，下載的檔案的標題是隨機數而不是校訂 ID。
